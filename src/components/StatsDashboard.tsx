        ...
        <div className="space-y-6">
          ...
          {totalMatches === 0 ? (
            ...
            <div className="space-y-6">
              ...
              {/* SECTION B.2: CHARTS FOR GOAL TYPES */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                {/* Visual Chart 4: Goal Types Local */}
                <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="w-2 md:w-2.5 h-5 bg-emerald-500 rounded-full"></span>
                    <h4 className="text-xs font-black uppercase text-slate-800 tracking-wider">
                      Tipos de Goles (A Favor)
                    </h4>
                  </div>
                  <div className="h-56 flex items-center justify-center">
                    {pieDataGoalTypesLocal.length === 0 ? (
                      <div className="text-center text-slate-400 p-4">
                        <Trophy size={28} className="mx-auto text-slate-300 mb-1 opacity-50" />
                        <p className="text-[11px] font-bold">Sin goles registrados</p>
                      </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          >
                            {pieDataGoalTypesLocal.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                            ))}
                          </Pie>
                          <Tooltip formatter={(value) => [`${value} goles`, 'Cantidad']} />
                        </PieChart>
                      </ResponsiveContainer>
                    )}
                  </div>
                ...
                {/* Visual Chart 5: Goal Types Rival */}
                <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="w-2 md:w-2.5 h-5 bg-rose-500 rounded-full"></span>
                    <h4 className="text-xs font-black uppercase text-slate-800 tracking-wider">
                      Tipos de Goles (En Contra)
                    </h4>
                  </div>
                  <div className="h-56 flex items-center justify-center">
                    {pieDataGoalTypesRival.length === 0 ? (
                      <div className="text-center text-slate-400 p-4">
                        <Trophy size={28} className="mx-auto text-slate-300 mb-1 opacity-50" />
                        <p className="text-[11px] font-bold">Sin goles registrados</p>
                      </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            ...
                            label={({ name, percent }) => `${name} (${percent}%)`}
                          >
                            {pieDataGoalTypesRival.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                            ))}
                          </Pie>
                          <Tooltip formatter={(value) => [`${value} goles`, 'Cantidad']} />
